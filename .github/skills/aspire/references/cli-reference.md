...
**Tested against:** Aspire CLI 13.3.0-preview.1 (daily channel)

> MCP command migration in 13.3 daily: prefer `aspire agent init` and `aspire agent mcp`. Legacy `aspire mcp init` and `aspire mcp start` remain available as deprecated aliases.
...
Browse and search Aspire documentation from `aspire.dev`.

```bash
aspire docs list
aspire docs search "mcp"
aspire docs get configure-the-mcp-server
```
...
```bash
# Initialize agent/MCP configuration
aspire agent init

# Start MCP server (invoked by MCP host)
aspire agent mcp
```

### `aspire telemetry`

View telemetry from the Dashboard API.

```bash
aspire telemetry logs <resource>
aspire telemetry spans <resource>
aspire telemetry traces <resource>
```

### `aspire mcp`

Manage the MCP (Model Context Protocol) server (legacy command group).

```bash
aspire mcp <subcommand>

# Subcommands:
#   init    Deprecated alias for `aspire agent init`
#   start   Deprecated alias for `aspire agent mcp`
```
...
# Deprecated: use `aspire agent init`.
...
# Deprecated: use `aspire agent mcp`.
...
The following commands are **not valid** in Aspire CLI 13.3 daily. Use alternatives:
