...
> **Note:** `aspire agent init` uses interactive prompts. If an agent terminal has prompt issues, run it in a normal interactive terminal.

---

## Understanding the Configuration

When you run `aspire agent init`, the CLI creates configuration files appropriate for your detected environment.
...
> **Version gate:** These tools were added in [PR #14028](https://github.com/dotnet/aspire/pull/14028) and ship in Aspire CLI **13.2+**. On older builds, these tools may be missing. Use daily channel when needed: `aspire update --self --channel daily`.
...
In 13.3 daily, these legacy commands still work but are deprecated:

- `aspire mcp init` -> use `aspire agent init`
- `aspire mcp start` -> use `aspire agent mcp`
...
The `aspire agent init` command supports:
...
5. **Search docs (13.2+):**
   ...
   - _(Requires CLI 13.2+. On older builds, use Context7 or `list_integrations`/`get_integration_docs` for integration-specific docs.)_
