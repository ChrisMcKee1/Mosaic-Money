...
- Keep Clerk and Plaid sandbox operator credentials only in local ignored `src/MosaicMoney.Web/triage-idp-plaid.env.local`; required keys include `MM_CLERK_FRONTEND_API_URL`, `MM_CLERK_BACKEND_API_URL`, `MM_CLERK_API_VERSION`, `MM_CLERK_SECRET_KEY`, `MM_CLERK_USER_A_EMAIL`, `MM_CLERK_USER_B_EMAIL`, `MM_CLERK_USER_A_USERNAME`, `MM_CLERK_USER_B_USERNAME`, `MM_CLERK_TEST_PASSWORD`, `MM_PLAID_CLIENT_ID`, `MM_PLAID_SANDBOX_SECRET`, `MM_PLAID_TEST_USERNAME`, and `MM_PLAID_TEST_PASSWORD`, with Plaid references `https://plaid.com/docs/sandbox/test-credentials/` and `https://plaid.com/docs/sandbox/user-custom/`.
...
The most important step is at the end of every single task that's ever run. All on uncommitted work even if you did not touch the file should be reviewed by your subagent code-simplifier before being committed. This is to ensure that all code adheres to our standards for readability, maintainability, and security. Do not skip this step. Always run the code-simplifier agent on all changed files before committing, even if you think the changes are minor or you didn't directly edit the file. This is a critical guardrail to maintain code quality across the entire codebase.