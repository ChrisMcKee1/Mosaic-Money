...
| MM-BE-02 | Ledger domain model baseline | MM-BE-01 | Core single-entry entities with `UserNote` and `AgentNote` fields separated. | Done |
| MM-BE-03 | PostgreSQL schema and extension migration | MM-BE-02 | EF migration enables `pgvector` and `azure_ai` with required indexes. | Done |
| MM-BE-04 | Minimal API contract v1 | MM-BE-02 | Resource endpoints/DTOs and validation behavior for MVP surface. | Done |
| MM-AI-01 | Classification outcome contract | MM-BE-04 | Stage, confidence, rationale, and review routing structure (no transcript storage). | Done |
| MM-AI-02 | AI workflow integration checks | MM-BE-01, MM-BE-03 | AI paths conform to same DB integration and orchestration constraints. | Done |
...
| MM-FE-03 | Responsive app shell and navigation | MM-FE-01 | Initial route shell for Dashboard, Transactions, NeedsReview. | Done |
| MM-MOB-01 | Shared domain contracts and API client | MM-BE-04 | Mobile consumes shared schema contracts aligned to backend payloads. | Done |
